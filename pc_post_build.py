...
# Build Doxygen
logger.info(os.path.join(configuration.project_path, 'documentation/doxygen/iondb_template'))
...
# Run Cppcheck
logger.info(os.path.join(configuration.project_path, 'src'))
...
with open(os.path.join(configuration.pc_output_path, 'cppcheck.xml'), 'w') as err:
	proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=err, universal_newlines=True)
	helper_functions.process_output_stream(proc)
